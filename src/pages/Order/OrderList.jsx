...
const OrderList = () => {
    ...
    const fetchResources = async () => {
      ...
      try {
        const [companiesRes, productsRes] = await Promise.all([
          ...
          productApi.getProducts({ limit: 5, search: productSearch })
        ]);
  ...
  // Fetch technicians when company changes or search changes
  useEffect(() => {
    ...
    fetchTechnicians();
  }, [draftForm.company, techSearch]);

  // Update itemCache when filteredTechnicians or products change (optional, but helps)
  useEffect(() => {
  ...
  const handleEditDraftOpen = (order) => {
    ...
    if (order.technician) {
      setItemCache(prev => ({ ...prev, [order.technician._id]: order.technician }));
    }
      ...
      <Box sx={modalStyle}>
        ...
        <form onSubmit={createDraftOrder}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={6}>
              <Autocomplete
                options={[...(draftForm.technician ? [itemCache[draftForm.technician]] : []), ...filteredTechnicians].filter((t, i, self) => t && self.findIndex(s => s._id === t._id) === i)}
                getOptionLabel={(option) => option.name || ''}
                value={itemCache[draftForm.technician] || null}
                onChange={(event, newValue) => {
                  if (newValue) setItemCache(prev => ({ ...prev, [newValue._id]: newValue }));
                  setDraftForm({ ...draftForm, technician: newValue ? newValue._id : '' });
                }}
            ...
            <Grid item xs={12}>
              ...
              {completeForm.products.map((product, index) => (
                <Grid container spacing={2} key={index} sx={{ mt: 1 }}>
                  <Grid item xs={6}>
                    <Autocomplete
                      options={[...(product.product ? [itemCache[product.product]] : []), ...products].filter((p, i, self) => p && self.findIndex(s => s._id === p._id) === i)}
                      getOptionLabel={(option) => {
                        if (typeof option === 'string') return option;
                        return option.name ? `${option.name} (₹${option.price})` : '';
                      }}
                      value={itemCache[product.product] || null}
                      onChange={(event, newValue) => {
                        if (newValue) setItemCache(prev => ({ ...prev, [newValue._id]: newValue }));
                        handleProductChange(index, 'product', newValue ? newValue._id : '');
                      }}
                      onInputChange={(event, newInputValue) => {
  ...
  return (
    <Box sx={{ width: '100%', p: 2, display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      {/* Set a fixed height for header section to allow grid to take remaining space */}
      <Box sx={{ flexShrink: 0 }}>
        ...
        >
          <TextField
            label="Search TCR / Customer / Phone"
            variant="outlined"
            size="small"
            value={searchInput}
      ...
      {/* Grid container taking full remaining space */}
